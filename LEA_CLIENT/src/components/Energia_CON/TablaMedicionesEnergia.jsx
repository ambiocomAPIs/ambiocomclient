...
import GraficaConsumoDiarioEnergia from "./Utils_Energia/GraficaConsumoDiarioEnergia";
...
export default function TablaMedicionesAgua() {
  ...
  const guardarColumna = async () => {
    ...
    setNuevaColumna({ nombre: "", key: "", unidad: "KWatts" });
  ...
  const renderConsumo = (row, index, key) => {
    ...
    const color =
      diff === 0 ? "#757575" : diff > CONSUMO_ALTO ? "#2E7D32" : "#C62828";

    if (!verConsumo) {
      return (
        <TooltipConsumo title={`Consumo diario: ${diff} KWatts`}>
          <span>{actual}</span>
        </TooltipConsumo>
      );
    }

    return <Typography sx={{ fontWeight: 600, color }}>{diff}</Typography>;
  };
    ...
    <Box sx={{ p: 3 }}>
      ...
      >
        <Typography variant="h4" sx={{ color: "#1A237E" }}>
          Medición diaria de contadores de Energia
        </Typography>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          ...
          >
            {verConsumo ? "Ver lecturas" : "Ver comparativo diario"}
          </Button>
        ...
        <Table size="small" stickyHeader>
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              {columnas.map((c) => (
                <TableCell key={c.key} align="center">
                  <b>{calcularAcumulado(c.key)} KWatts</b>
                </TableCell>
              ))}
      ...
      <Dialog open={openColumna} fullWidth maxWidth="xs">
        <DialogTitle>Nuevo medidor de Energia</DialogTitle>
      ...
      >
        ...
        >
          ...
          <Typography sx={{ fontWeight: 960 }}>
            Gráfica de consumo diario de ENERGIA
          </Typography>
        ...
        >
          <GraficaConsumoDiarioEnergia
            mediciones={medicionesLectura}
            columnas={columnas}
          />
        </Box>
      </Dialog>
