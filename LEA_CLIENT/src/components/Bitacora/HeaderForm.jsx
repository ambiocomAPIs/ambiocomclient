...
function HeaderForm({ data, onChange, clearFieldsExceptFechaTurno, trabajadoresRegistradosContext }) {
  ...
  useEffect(() => {
    const fetchExistingData = async () => {
      ...
      try {
        const response = await axios.get("https://ambiocomserver.onrender.com1/api/bitacora/getbyfechayturno", {
          params: {
            fecha: data.fecha,
            turno: data.turno,
          },
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.post("https://ambiocomserver.onrender.com1/api/bitacora/bitacorareplaceall", [data]);
