...
import {calcularDiferenciaEnMeses} from './Utils_Insumos/Funcions_Insumos/CalcularDiferenciaFechas'
import {ExportExcelWithTemplate} from '../../utils/Functions/DownloadExcelData'
import DataTablePieChartInsumosModal from './Utils_Insumos/Modals_Insumos/DataTablePieChartInsumosModal'
import DataTableChartModalCost from './Utils_Insumos/Modals_Insumos/DataTableChartModalCost'
import ModalFilterData from './Utils_Insumos/Modals_Insumos/ModalFilterData';
import ReportarConsumoModal from './Utils_Insumos/Modals_Insumos/ReportarConsumoModal';
import DetallesMovimientos from './Utils_Insumos/Modals_Insumos/DetallesMovimientos'
import DataTableChartModalInventariovsSAP from './Utils_Insumos/Modals_Insumos/DataTableChartModalInventariovsSAP'
import DataTableChartModalCostMensual from './Utils_Insumos/Modals_Insumos/DataTableChartModalCostMensual'
...
import FileUpload from './Utils_Insumos/page/UploadFile';
...
// abre modal con el tipodeoperacion
  ...
  const verificarImagen = async (id) => {
    if (!id) return false;
    try {
      const res = await fetch(`https://ambiocomserver.onrender.com1/imagenes/${id}.jpg`, { method: 'HEAD' });
  ...
  useEffect(() => {
    ...
    axios.get('https://ambiocomserver.onrender.com1/api/table/data')
      .then(response => {
  ...
  const DownloadPdf = async (rowId) => {
    try {
        ...
        const response = await axios.get(`https://ambiocomserver.onrender.com1/api/pdfs/download/${rowId}`);
...
const DeletePdf = async (rowId) => {
  try {
    ...
    if (result.isConfirmed) {
      const response = await axios.delete(`https://ambiocomserver.onrender.com1/api/pdfs/${rowId}`);
...
const NotificarAlerta = async (params) => {
  try {
    ...
    // Verifica si el usuario confirmó la acción
    if (result.isConfirmed) {
      const response = await axios.get(`https://ambiocomserver.onrender.com1/api/email/notificar-producto/${params._id}`);
  ...
  const handleBlur = async () => { 
    ...
    try {
      ...
      const response = await axios.post('https://ambiocomserver.onrender.com1/api/table/datareplaceall', newData);  
  ...
  const agregarDataFila = ()=> {
    ...
    axios.post('https://ambiocomserver.onrender.com1/api/table/data', newFile)
    .then(response => {
...
const deleteRowData = (rowId) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      axios.delete(`https://ambiocomserver.onrender.com1/api/table/data/${rowId}`)
        .then(() => {
...
const clickColumFixed = (columnClicked) => {
  ...
  const guardarCierreMes = async (cierreData) => {
    const response = await fetch('https://ambiocomserver.onrender.com1/api/cierreMes/data', {
      method: 'POST',
      headers: {
    ...
    const DescargarManual = async () => {
      try {
        ...
        const response = await axios.get('https://ambiocomserver.onrender.com1/api/download/downloadmanual', {
    ...
    const handleImageUpload = async (id) => {
      ...
      try {
        ...
        const exists = await fetch(`https://ambiocomserver.onrender.com1/imagenes/${id}.jpg`, { method: 'HEAD' })
          .then(res => res.ok)
          .catch(() => false);
        ...
        if (exists) {
          return Swal.fire({
            ...
            imageUrl: `https://ambiocomserver.onrender.com1/imagenes/${id}.jpg`,
            imageWidth: 400,
            imageHeight: 350,
        ...
        const response = await fetch(`https://ambiocomserver.onrender.com1/upload/${id}`, {
          method: 'POST',
          body: formData
        });
      ...
      Array.isArray(filteredData) && filteredData.length > 0 ? (
        ...
        filteredData.map((row, rowIndex) => {    
          ...
          if (['administrativo'].includes(usuario?.rol)) { // todas menos 19 y 20 inventario critico y SAP
            BlockedColumns = [2,4,5,13,14,15,16,17,21, 22, 23, 24,25];
